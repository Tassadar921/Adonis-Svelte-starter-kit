...
<Link
    ...
    className={`${!notClickable ? (footer ? 'hover:bg-gray-400' : 'hover:bg-gray-600') : 'cursor-not-allowed opacity-50'} ${footer ? 'flex justify-center' : ''} px-2 flex transition-colors duration-300 rounded ${notClickable ? '' : 'cursor-pointer dark:hover:bg-gray-700'}`}
    {target}
    on:click
>
